function xgd_table = xgendatag(gen, inp)
...
%   - wear-and-tear costs determined by fuel type (three tiers)
...
%   - PositiveActiveReservePrice, PositiveLoadFollowReserveQuantity and negative for gens proportinal to PMAX, greater than or equal to 0
...
rpraw = zeros(size(mpc.gen, 1), 1); % potentially different wear-and-tear (ramp) costs
rpraw(bl, 1) = 0.009;
rpraw(sl, 1) = 0.006;
rpraw(pl, 1) = 0.003;

%%-----  contingency reserve prices -----
crespf = 1.001;                     % price for PositiveActiveReservePrice, NegativeActiveReservePrice, gens
cresp = 0.001;                      % price for PositiveActiveReservePrice, NegativeActiveReservePrice, not gens
crpr = ones(size(mpc.gen, 1), 1)*cresp;
crpr([bl; sl; pl], 1) = crespf;     % price for positive reserves
